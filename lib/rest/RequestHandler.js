    ...
    */
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(this.delayLock && method === "PATCH" && url.startsWith("/channels/")) {
                reject(new Error(`delayLock for channels is on, ignored ${method} ${url}`));
                return;
            }
            ...
            const actualCall = (cb) => {
                ...
                req.setTimeout(this.options.requestTimeout, () => {
                    if(delayedRequests.length >= 5) {
                        const first = delayedRequests[0];
                        const last = delayedRequests[4];

                        if((last - first) > 60000) {
                            this.delayLock = true;
                            setTimeout(() => this.delayUnlock(), 150000);
                        } else {
                            delayedRequests.shift();
                        }
                    }

                    delayedRequests.push(Date.now())
            ...
            if(this.globalBlock && auth) {
                if(this.readyQueue.length > 25) {
                    if(this.readyQueue.length > 30){
                        reject(new Error(`readyQueue is bigger then 30 on ${method} ${url}`));
                        return;
                    }
