...
class RequestHandler {
    constructor(client, options) {
        ...
        this._client = client;
        this.userAgent = `ServerStats (Eris version: ${require("../../package.json").version})`;
        this.ratelimits = {};
        this.latencyRef = {
    ...
    */
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            if(this.globalBlock && method === "PATCH" && url.startsWith("/channels/")) {
                reject(new Error(`Channels update dropped because of globalBlock`));
                return;
            }

            let attempts = 0;
            const actualCall = (cb, createDate) => {
                if(Date.now() - createDate > this.options.requestTimeout) {
                    cb();
                    reject(new Error(`Request ran after timeout passed, rejected ${method} ${url}`));
                    return;
                }
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.statusCode >= 300) {
                            if (this._client.stats) {
                                if (!this._client.stats.errorCodes) this._client.stats.errorCodes = {};

                                if (!this._client.stats.errorCodes[resp.statusCode]) {
                                    this._client.stats.errorCodes[resp.statusCode] = 1;
                                } else {
                                    this._client.stats.errorCodes[resp.statusCode]++;
                                }
                            }

                            if(resp.statusCode === 429) {
                                if(this._client.stats) {
                                    this._client.stats.ratelimited ? this._client.stats.ratelimited++ : this._client.stats.ratelimited = 1;
                                }

                                cb();
                                reject(new Error(`Error 429 (╯°□°）╯︵ ┻━┻, ratelimit on ${method} ${url}`));
                                return;
                            } else if(resp.statusCode === 502 && ++attempts < 3) {
                                setTimeout(() => {
            ...
            if(this.globalBlock && auth) {
                reject(new Error(`Request dropped because of globalBlock`));
            } else {
    ...
    routefy(url, method) {
        ...
        if(method === "GET" && /\/guilds\/[0-9]+\/channels$/.test(route)) {
