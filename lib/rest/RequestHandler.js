    ...
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            if(this.globalBlock && method === "PATCH" && url.startsWith("/channels/")) {
                reject(new Error(`Channels ${method} ignored because of ratelimit ${url}`));
                return;
            }
            ...
            const actualCall = (cb, createDate) => {
                ...
                req.setTimeout(this.options.requestTimeout, () => {
                    reqError = new Error(`Eris | Request timed out (>${this.options.requestTimeout}ms) on ${method} ${url}`);
                    req.abort();
                });
            ...
            if(this.globalBlock && auth) {
                if(this.readyQueue.length > 25) {
                    if(this.readyQueue.length > 30){
                        reject(new Error("readyQueue is bigger then 30"));
                        return;
                    }
