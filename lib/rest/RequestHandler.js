    ...
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(this.delayLock && method === "PATCH" && url.startsWith("/channels/")) {
                reject(new Error("delayLock for channels is on"));
                return;
            }
            ...
            const actualCall = (cb) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.statusCode >= 300) {
                            if(resp.statusCode === 429) {
                                ...
                                if(method === "PATCH" && url.startsWith("/channels/")) {
                                    reject(new Error("Error 429 (╯°□°）╯︵ ┻━┻, rate limit"));
                                    return;
                                }
                ...
                req.setTimeout(this.options.requestTimeout, () => {
                    this.delayLock = true;
                    setTimeout(() => this.delayUnlock(), 300000);
