    ...
    */
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            if((this.delayLock || this.globalBlock) && method === "PATCH" && url.startsWith("/channels/")) {
                reject(new Error(`${this.delayLock ? "delayLock" : "globalBlock"} for channels is on, ignored ${method} ${url}`));
                return;
            }
            ...
            const actualCall = (cb) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.statusCode >= 300) {
                            if(resp.statusCode === 429) {
                                ...
                                reject(new Error(`Error 429 (╯°□°）╯︵ ┻━┻, rate limit on ${method} ${url}`));
                                return;
                ...
                req.setTimeout(this.options.requestTimeout, () => {
                    if(this.delayedRequests.length >= 6) {
                        const first = this.delayedRequests[0];
                        const last = this.delayedRequests[5];

                        if((last - first) > 30000) {
                            if(!this.delayLock) {
                                this.delayLock = true;
                                setTimeout(() => this.delayUnlock(), 60000);
                            }
                        }

                        this.delayedRequests.shift();
                    }

                    this.delayedRequests.push(Date.now());
                    reqError = new Error(`Request timed out in RH (>${this.options.requestTimeout}ms) on ${method} ${url}`);
                    req.abort();
                });
