...
class Interaction extends Base {
    ...
    */
	async createFollowup(content) {
		...
		return this._client.requestHandler.request(
			'POST',
			Endpoints.WEBHOOK_TOKEN(this.applicationId, this.token),
			true,
			content,
			content.file
		); //.then((response) => new Message(response, this)); Errored out?
	}
    ...
    */
	async delete(messageId) {
		if (!messageId)
			return this._client.requestHandler.request(
				'DELETE',
				Endpoints.WEBHOOK_MESSAGE(this.applicationId, this.token, messageId),
				true
			);
	}
