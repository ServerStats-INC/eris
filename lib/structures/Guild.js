...
class Guild extends Base {
    constructor(data, {client, shardId}) {
        ...
        this.members = new Collection(Member); // Maybe check if we can remove user object from members
        ...
        this.hasCached = {
            members: false,
            dynamic: false
            // cacheFilledAt: 0 // Now date + maybe a week or so
        }
        this.cacheSettings = {
            hasSetup: false,
            hasPending: false,
        ...
        this.savedCount = {
            // lastUpdated: 0, Track last time the bot updated (also for cache remove)
    ...
    async fillBotsSet(setSetting) {
        console.log(`${this.id} | fillBotsSet ran in updator`);
        ...
        if(this.cacheSettings.hasSetup) {
            ...
            if(!this.hasCached.members) {
                console.log(`${this.id} | fillBotsSet ran a member fetch`);
                if(this.savedCount.bots.size > 0) this.savedCount.bots.clear();
                await this.fetchAllMembers(20000, "fillBotsSet");
            }
    ...
    async fillPendingSet(setSetting) {
        console.log(`${this.id} | fillPendingSet ran in updator`);
        ...
        if(this.cacheSettings.hasPending) {
            ...
            if(!this.hasCached.members) {
                console.log(`${this.id} | fillPendingSet ran a member fetch`);
                if(this.savedCount.pending.size > 0) this.savedCount.pending.clear();
                await this.fetchAllMembers(20000, "fillPendingSet");
            }
    ...
    async fillNoRoleCache(setSetting) {
        console.log(`${this.id} | fillNoRoleCache ran in updator`);
        ...
        if(this.cacheSettings.hasNoRole) {
            ...
            if(!this.hasCached.members) {
                console.log(`${this.id} | fillNoRoleCache ran a member fetch`);
                if(this.membersCache.size > 0) this.membersCache.wipe();
                await this.fetchAllMembers(20000, "fillNoRoleCache");
            }

            if(!this.hasCached.dynamic) {
                this.hasCached.cacheFilledAt = Date.now();
    ...
    async fillRoleCache(roles) {
        console.log(`${this.id} | fillRoleCache ran in updator`);
        if(roles) {
            roles.forEach((r) => {
        ...
        if(this.cacheSettings.roleCounter.size > 0) {
            if(!this.membersCache) this.membersCache = new Collection(MemberCache);
            if(!this.hasCached.members) {
                console.log(`${this.id} | fillRoleCache ran a member fetch`);
                if(this.membersCache.size > 0) this.membersCache.wipe();
                await this.fetchAllMembers(20000, "fillRoleCache");
            }
    ...
    async clearAllSetAndCache() {
        this.cacheSettings.hasWelcome = false;
        this.clearBotsSet();
        this.clearPendingSet();
        this.clearNoRoleCache();
        this.clearRoleCache();

        this.clearAllMembers(); // Just to make sure it's cleared as well
    }
    ...
    clearNoRoleCache() {
        ...
        if(this.cacheSettings.roleCounter.size === 0 && !this.cacheSettings.hasWelcome) {
            if(this.membersCache) {
                this.membersCache.wipe();
                this.hasCached.cacheFilledAt = undefined;
    ...
    clearRoleCache() {
        ...
        if(!this.cacheSettings.hasNoRole && !this.cacheSettings.hasWelcome) {
            if(this.membersCache) {
                this.membersCache.wipe();
                this.hasCached.cacheFilledAt = undefined;
                this.hasCached.dynamic = false;
    ...
    */
    clearAllMembers(hardReset) {
        ...
        console.log(`${this.id} | clearAllMembers ran and cache is ${newCount}`);
        return newCount;
    }
