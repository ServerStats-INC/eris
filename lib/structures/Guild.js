...
class Guild extends Base {
            ...
            for(const voiceState of data.voice_states) {
                try {
                    ...
                    if(channel && channel.voiceMembers) {
                        channel.voiceMembers.add(voiceState.user_id);
                    }
                } catch(err) {
    ...
    async fillBotsSet(needUpdate) {
        this.cacheSettings.hasSetup = true;
        if((this.cacheSettings.hasSetup && (!this.savedCount.bots || this.savedCount.bots.size < 1)) || needUpdate) {
            if(!this.savedCount.bots) this.savedCount.bots = new Set();
            if(!this.hasCached.members || this.members.size < 2) {
                if(this.savedCount.bots.size > 0) this.savedCount.bots.clear();
                await this.fetchAllMembers(15000, "fillBotsSet").catch(() => {
                    this.savedCount.failedFetch++;
                    return this.clearBotsSet();
                });
            }

            if(!this.hasCached.cacheFilledAt) this.hasCached.cacheFilledAt = Date.now();
            this.members.filter((m) => m.bot).forEach((m) => {
                this.savedCount.bots.add(m.id);
            });
        }
    }

    async fillStatusSet(needUpdate) {
        this.cacheSettings.hasStatus = true;
        if((this.cacheSettings.hasStatus && (!this.membersCache || this.membersCache.size < 1)) || needUpdate) {
            if(!this.membersCache) this.membersCache = new Collection(MemberCache);
            if(!this.hasCached.members || this.members.size < 2) {
                if(this.membersCache.size > 0) this.membersCache.wipe();
                await this.fetchAllMembers(15000, "fillStatusSet").catch(() => {
                    this.savedCount.failedFetch++;
                    return this.clearStatusCache();
                });
            }

            if(!this.hasCached.cacheFilledAt) this.hasCached.cacheFilledAt = Date.now();    
            this.hasCached.dynamic = true;
            this.members.forEach((m) => {
                this.membersCache.update(m, this.cacheSettings);
            });
        }
    }

    async fillActivitiesSet(needUpdate) {
        this.cacheSettings.hasActivities = true;
        if((this.cacheSettings.hasActivities && (!this.membersCache || this.membersCache.size < 1)) || needUpdate) {
            if(!this.membersCache) this.membersCache = new Collection(MemberCache);
            if(!this.hasCached.members || this.members.size < 2) {
                if(this.membersCache.size > 0) this.membersCache.wipe();
                await this.fetchAllMembers(15000, "fillActivitiesSet").catch(() => {
                    this.savedCount.failedFetch++;
                    return this.clearActivitiesCache();
                });
            }
    ...
    async fillPendingSet(needUpdate) {
        this.cacheSettings.hasPending = true;
        if((this.cacheSettings.hasPending && (!this.savedCount.pending || this.savedCount.pending.size < 1)) || needUpdate) {
            if(!this.savedCount.pending) this.savedCount.pending = new Set();
            if(!this.hasCached.members || this.members.size < 2) {
                if(this.savedCount.pending.size > 0) this.savedCount.pending.clear();
                await this.fetchAllMembers(15000, "fillPendingSet").catch(() => {
                    this.savedCount.failedFetch++;
                    return this.clearPendingSet();
                });
            }

            if(!this.hasCached.cacheFilledAt) this.hasCached.cacheFilledAt = Date.now();
            this.members.filter((m) => m.pending).forEach((m) => {
                this.savedCount.pending.add(m.id);
            });
        }
    }

    async fillNoRoleCache(needUpdate) {
        this.cacheSettings.hasNoRole = true;
        if((this.cacheSettings.hasNoRole && (!this.membersCache || this.membersCache.size < 1)) || needUpdate) {
            if(!this.membersCache) this.membersCache = new Collection(MemberCache);
            if(!this.hasCached.members || this.members.size < 2) {
                if(this.membersCache.size > 0) this.membersCache.wipe();
                await this.fetchAllMembers(15000, "fillNoRoleCache").catch(() => {
                    this.savedCount.failedFetch++;
                    return this.clearNoRoleCache();
                });
            }
            ...
            this.hasCached.dynamic = true;
            this.members.forEach((m) => {
                if(this.cacheSettings.hasWelcome) {
                    this.membersCache.update(m, this.cacheSettings);
                } else {
                    if(m.roles.length < 1) {
                        this.membersCache.update(m, this.cacheSettings);
    ...
    async fillRoleCache(roles) {
        const oldRolesSize = this.cacheSettings.roleCounter.size;
        if(roles) {
            roles.forEach((r) => {
                this.cacheSettings.roleCounter.add(r);
            });
        }
        
        if((this.cacheSettings.roleCounter.size > 0 && (!this.membersCache || this.membersCache.size < 1)) || (this.cacheSettings.roleCounter.size !== oldRolesSize)) {
            if(!this.membersCache) this.membersCache = new Collection(MemberCache);
            if(!this.hasCached.members || this.members.size < 2) {
                if(this.membersCache.size > 0 && oldRolesSize !== this.cacheSettings.roleCounter.size) {
                    this.membersCache.wipe();
                    await this.fetchAllMembers(15000, "fillRoleCache").catch(() => {
                        this.savedCount.failedFetch++;
                        return this.clearRoleCache();
                    });
                } else if (this.membersCache.size < 1) {
                    await this.fetchAllMembers(15000, "fillRoleCache").catch(() => {
                        this.savedCount.failedFetch++;
                        return this.clearRoleCache();
                    });
            ...
            this.members.forEach((m) => {
                if(this.cacheSettings.hasWelcome) {
                    ...
                    if(!!m.roles.find((r) => this.cacheSettings.roleCounter.has(r))) {
                        this.membersCache.update(m, this.cacheSettings);
                    }
                }
            });
    ...
    async clearAllSetAndCache(hardReset) {
        this.cacheSettings.hasSetup = false;
        this.cacheSettings.hasPending = false;
        this.cacheSettings.hasNoRole = false;
        this.cacheSettings.hasWelcome = false;
        this.cacheSettings.hasStatus = false;
        this.cacheSettings.hasActivities = false;
        this.cacheSettings.roleCounter.clear();

        if(this.bans) {
            this.bans = false;
        }

        if(this.savedCount.bots) {
            this.savedCount.bots.clear();
            delete this.savedCount.bots;
        }

        if(this.savedCount.pending) {
            this.savedCount.pending.clear();
            delete this.savedCount.pending;
        }

        if(this.membersCache) {
            this.membersCache.wipe();
            delete this.membersCache;
        }

        this.hasCached.dynamic = false;
        delete this.hasCached.cacheFilledAt;
        this.clearAllMembers(hardReset); // Just to make sure it's cleared as well
    }
    ...
    clearStatusCache() {
        this.cacheSettings.hasStatus = false;
        if(this.cacheSettings.roleCounter.size < 1 && !this.cacheSettings.hasWelcome && !this.cacheSettings.hasActivities && !this.cacheSettings.hasNoRole) {
            this.hasCached.dynamic = false;
            if(this.membersCache) {
                this.membersCache.wipe();
                delete this.membersCache;
    ...
    clearActivitiesCache() {
        this.cacheSettings.hasActivities = false;
        if(this.cacheSettings.roleCounter.size < 1 && !this.cacheSettings.hasWelcome && !this.cacheSettings.hasStatus && !this.cacheSettings.hasNoRole) {
            this.hasCached.dynamic = false;
            if(this.membersCache) {
                this.membersCache.wipe();
                delete this.membersCache;
    ...
    clearNoRoleCache() {
        this.cacheSettings.hasNoRole = false;
        if(this.cacheSettings.roleCounter.size < 1 && !this.cacheSettings.hasWelcome && !this.cacheSettings.hasStatus && !this.cacheSettings.hasActivities) {
            this.hasCached.dynamic = false;
            if(this.membersCache) {
                this.membersCache.wipe();
                delete this.membersCache;
    ...
    clearRoleCache() {
        this.cacheSettings.roleCounter.clear();
        if(!this.cacheSettings.hasNoRole && !this.cacheSettings.hasWelcome && !this.cacheSettings.hasStatus && !this.cacheSettings.hasActivities) {
            this.hasCached.dynamic = false;
            if(this.membersCache) {
                this.membersCache.wipe();
                delete this.membersCache;
    ...
    * @arg {String} query The query params used for looking up the bans
    * @returns {Promise<Array<Object>>} Resolves with an array of {reason: String, user: User}
    */
    getBans(query) {
        return this._client.getGuildBans.call(this._client, this.id, query);
    }
