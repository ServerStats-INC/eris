...
class Guild extends Base {
    ...
    async fillBotsSet(setSetting) {
        ...
        if(this.cacheSettings.hasSetup) {
            ...
            if(!this.hasCached.members || this.members.size < 2) {
                ...
                await this.fetchAllMembers(20000, "fillBotsSet").catch(() => {
                    return this.clearBotsSet();
                });
            }

            if(this.members.size !== this.memberCount) return this.clearBotsSet();
    ...
    async fillPendingSet(setSetting) {
        ...
        if(this.cacheSettings.hasPending) {
            ...
            if(!this.hasCached.members || this.members.size < 2) {
                ...
                await this.fetchAllMembers(20000, "fillPendingSet").catch(() => {
                    return this.clearPendingSet();
                });
            }

            if(this.members.size !== this.memberCount) return this.clearPendingSet();
            if(!this.hasCached.cacheFilledAt) this.hasCached.cacheFilledAt = Date.now();
    ...
    async fillNoRoleCache(setSetting) {
        ...
        if(this.cacheSettings.hasNoRole) {
            ...
            if(!this.hasCached.members || this.members.size < 2) {
                ...
                await this.fetchAllMembers(20000, "fillNoRoleCache").catch(() => {
                    return this.clearNoRoleCache();
                });
            }

            if(this.members.size !== this.memberCount) return this.clearNoRoleCache();
            if(!this.hasCached.cacheFilledAt) this.hasCached.cacheFilledAt = Date.now();
    ...
    async fillRoleCache(roles) {
        ...
        if(this.cacheSettings.roleCounter.size > 0) {
            ...
            if(!this.hasCached.members || this.members.size < 2) {
                if(this.membersCache.size > 0) this.membersCache.wipe();
                await this.fetchAllMembers(20000, "fillRoleCache").catch(() => {
                    return this.clearRoleCache();
                });
            }

            if(this.members.size !== this.memberCount) return this.clearRoleCache();
            if(!this.hasCached.cacheFilledAt) this.hasCached.cacheFilledAt = Date.now();
    ...
    clearNoRoleCache() {
        ...
        if(this.cacheSettings.roleCounter.size < 1 && !this.cacheSettings.hasWelcome) {
