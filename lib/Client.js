...
class Client extends EventEmitter {
    ...
    * @arg {Array} [options.enabledEvents] Array of event names that are allowed to be processed
    ...
    */
    constructor(token, options) {
        ...
        // Required events so force them to true
        if(!this.options.enabledEvents.includes("READY")) this.options.enabledEvents.push("READY");
        if(!this.options.enabledEvents.includes("RESUMED")) this.options.enabledEvents.push("RESUMED");
        if(!this.options.enabledEvents.includes("GUILD_CREATE")) this.options.enabledEvents.push("GUILD_CREATE");
        if(!this.options.enabledEvents.includes("GUILD_DELETE")) this.options.enabledEvents.push("GUILD_DELETE");
        if(!this.options.enabledEvents.includes("GUILD_MEMBERS_CHUNK")) this.options.enabledEvents.push("GUILD_MEMBERS_CHUNK");
    ...
    */
    ...
    // Store stuff, because Discord is a fucking bitch!! Fuck you Discord
    // =====================================================================
    ...
    */
    ...
    /**
    * Gets a subscription to a SKU
    * @param {String} skuID The ID of the SKU
    * @param {String} subscriptionID The ID of the subscription
    * @returns {Promise<Subscription>}
    */
    getSKUSubscription(skuID, subscriptionID) {
        return this.requestHandler.request("GET", Endpoints.SKU_SUBSCRIPTION(skuID, subscriptionID), true).then((data) => new Subscription(data));
    }

    /**
    * Gets the list of subscriptions to a SKU
    * @param {String} skuID The ID of the SKU
    * @param {Object} options The options for the request
    ...
    */
    getSKUSubscriptions(skuID, options = {}) {
        options.user_id = options.userID;
        return this.requestHandler.request("GET", Endpoints.SKU_SUBSCRIPTIONS(skuID), true, options).then((data) => data.map((sub) => new Subscription(sub)));
    }

    // =====================================================================
    // End of store stuff, still fuck you Discord
    ...
    * @returns {Promise}
    */
    editChannelPosition(channelID, guildData, position, options = {}) {
        ...
        return this.requestHandler.request("PATCH", Endpoints.GUILD_CHANNELS(guildData.id), true, channels.map((channel, index) => ({
            id: channel.id,
            position: index + min,
    ...
    * @arg {String} guildID The ID of the guild
    * @arg {String} query The query string params 
    * @returns {Promise<Array<Object>>} Resolves with an array of {reason: String, user: User}
    */
    getGuildBans(guildID, query = "") {
        return this.requestHandler.request("GET", Endpoints.GUILD_BANS(guildID) + query, true).then((bans) => {
            bans.forEach((ban) => {
                ban.user = {
                    id: ban.user.id,
                    username: ban.user.username,
    ...
    */
    ...
    /**
    * Get all invites in a guild or 1 by code
    * @arg {String} guildID The ID of the guild
    * @arg {String} code Code from the invite you want
    * @returns {Promise<Array<Invite>>}
    */
    async getGuildInvites(guildID, code) {
        if(!code) {
        ...
        const reqInvite = await this.requestHandler.request("GET", Endpoints.GUILD_INVITES(guildID), true)
            .then((invites) => invites.filter((invite) => invite.code === code)[0]);
        return !reqInvite ? undefined : new Invite(reqInvite, this);
    }
