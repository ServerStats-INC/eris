    ...
    */
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "MESSAGE_CREATE": {
                if(packet.d.type !== 0 || packet.d.author.bot) {
                    break;
                }
            ...
            case "CHANNEL_CREATE": {
                if(![0, 2, 4, 5, 13].includes(packet.d.type)) {
                    break;
                }
                ...
                if(packet.d.guild_id) {
                    ...
                    if(!channel.guild.channels) {
                        this.emit("debug", `Received CHANNEL_CREATE but channels missing in guild ${packet.d.guild_id}`);
                        break;
                    }
            ...
            case "THREAD_CREATE": // Disable threads, we don't use them
            case "THREAD_UPDATE": 
            case "THREAD_DELETE": 
