...
class Shard extends EventEmitter {
    ...
    */
    disconnect(options = {}, error) {
        ...
        if(error) {
            this.emit("error", !error.message ? error : error.message, this.id);
        }
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "MESSAGE_CREATE": {
                if(packet.d.type !== 0 || packet.d.author.bot || packet.d.content.length <= 3) {
                    break;
                }
            ...
            case "GUILD_UPDATE": {
                ...
                // EDIT: Do something to this and the GUILD cache
                let oldGuild = null;
                oldGuild = {
                    name: guild.name,
                    region: guild.region,
            ...
            case "CHANNEL_CREATE": {
                if(![0, 2, 4, 5].includes(packet.d.type)) {
                    break;
                }
