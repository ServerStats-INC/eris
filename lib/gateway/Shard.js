...
class Shard extends EventEmitter {
    ...
    get commandTokens() {
        let result = {};
        const remaining = this.globalBucket.tokenLimit - this.globalBucket.reservedTokens - this.globalBucket.tokens;
        const queued = remaining == 0 ? this.globalBucket.queue.length : 0;

        if (remaining > 0) result.remaining = remaining;
    ...
    */
    disconnect(options = {}, error) {
        ...
        if(error) {
            if(![1001, 1002, 1006].includes(error.code)) {
    ...
    */
                ...
                if(packet.d.chunk_index >= packet.d.chunk_count - 1) {
                    if(this.requestMembersPromise.hasOwnProperty(packet.d.nonce)) {
                        ...
                        if(packet.d.members.length >= 2) {
                            ...
                            if(Math.abs(guild.memberCount - newCount) <= getOffset(newCount) && newCount >= 2) {
    ...
    _onWSClose(code, reason) {
        ...
        if(code) {
            if(this.client.stats) {
                if(!this.client.stats.closeCodes) this.client.stats.closeCodes = {};
                
                if(!this.client.stats.closeCodes[code]) {
                    this.client.stats.closeCodes[code] = 1;
                } else {
                    this.client.stats.closeCodes[code]++;
                }
            }
