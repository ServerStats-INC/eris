...
class Shard extends EventEmitter {
    ...
    */
    ...
    async identify() {
        ...
        if(this.client.options.useConcurrentBoot) {
    ...
    requestGuildMembers(guildID, options) {
        const opts = {
            ...
            presences: !this.client.options.intents || (this.client.options.intents && !(this.client.options.intents & Constants.Intents.guildPresences)) ? false : true
        };
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "PRESENCE_UPDATE": {
                ...
                if((guild.hasCached.dynamic && guild.membersCache) || packet.d.user.id === this.client.user.id) {
                    this.emit("presenceUpdate", guild, guild.membersCache.update(packet.d, guild.cacheSettings));
                } else {
                    this.emit("presenceUpdate", guild, packet.d);
                }
                break;
            }
            case "VOICE_STATE_UPDATE": { // (╯°□°）╯︵ ┻━┻
                ...
                // Add missing member if d.member is present
                packet.d.id = packet.d.user_id;
                if(guild.hasCached.members || packet.d.user_id === this.client.user.id) {
                    guild.members.add(packet.d, guild);
                }

                const channel = guild.channels.find((channel) => (channel.type === ChannelTypes.GUILD_VOICE || channel.type === ChannelTypes.GUILD_STAGE_VOICE) && channel.voiceMembers.has(packet.d.id));
                if(channel) {
                    channel.voiceMembers.delete(packet.d.id);
                }
                ...
                if(packet.d.channel_id && (newChannel = guild.channels.get(packet.d.channel_id)) && (newChannel.type === ChannelTypes.GUILD_VOICE || newChannel.type === ChannelTypes.GUILD_STAGE_VOICE)) { // Welcome to Discord, where one can "join" text channels{
                    ...
                    * @event Client#voiceChannelChange
                    * @prop {Member} member The member
                    * @prop {VoiceChannel | StageChannel} newChannel The voice channel
                    */
                    newChannel.voiceMembers.add(packet.d.id);
                    this.emit("voiceChannelChange", guild, newChannel, packet.d.id);
                }
                break;
            }
            case "GUILD_MEMBER_ADD": {
                ...
                if(guild.hasCached.dynamic && guild.membersCache) {
            ...
            case "GUILD_BAN_ADD": {
                ...
                */
                ...
                this.emit("guildBanAdd", guild, packet.d.user);
                break;
            }
            case "GUILD_BAN_REMOVE": {
                ...
                * @prop {Guild} guild The guild
                * @prop {User} user The unbanned user
                */
                const guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    break;
                ...
                if(guild.bans) guild.bans--;
                this.emit("guildBanRemove", guild, packet.d.user);
                break;
            }
            case "INVITE_CREATE": {
                const guild = this.client.guilds.get(packet.d.guild_id);
            ...
            case "ENTITLEMENT_CREATE": {
                /**
                 * Fires whenever an entitlement is created
            ...
            case "ENTITLEMENT_UPDATE": {
                /**
                 * Fires whenever an entitlement is updated, for example, whenever the subscriber renews or cancels their subscription
            ...
            case "ENTITLEMENT_DELETE": {
                /**
                 * Fires whenever an entitlement is deleted for reasons other than expiry
            ...
            case "SUBSCRIPTION_CREATE": {
                 ...
                 * @prop {Subscription} subscription The created subscription
                 */
