...
class Redis {
    ...
    lock(key, expiry = 60e3, timeout = 400) {
        if(!key.startsWith("lock:")) {
            key = `lock:${this.processName}:${key}`;
        }
        ...
        return new Promise(async (resolve) => {
            ...
            if(didSet) {
                ...
                timeout = Math.min(Math.round(timeout * (Math.random() * 2 + 1)), 10000);

                setTimeout(() => {
                    resolve(this.lock(key, expiry, timeout));
                }, timeout);
            }
        });
    }
